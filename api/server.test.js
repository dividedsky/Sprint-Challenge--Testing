...
describe('/POST endpoint', () => {
  it('should return an error if the object shape is invalid', async () => {
    try {
      ...
      expect(response.body).toEqual({
        error: 'Game must have a title and release year!',
      });
      body = {
        releaseYear: 1986,
        genre: 'Fantasy',
  ...
  it('should return 400 if the title is a duplicate', async () => {
    try {
      const body = {
        title: 'Test Game',
        genre: 'RPG',
      ...
      expect(response.body).toEqual({ error: 'A game with that title already exists' });
    } catch (err) {
      console.log(err);
    }
  });
